...
export default class SessionManager extends EventEmitter {
  ...
  async start(sessionId) {
    ...
    if (entry.status === "starting" || entry.restarting) {
      if (!entry.sock) {
        logger.debug(
          { sessionId },
          "[manager] start() called while already starting, sock not ready yet"
        );
        return null;
      }
      return entry.sock;
  ...
  _attachSocketListeners(sessionId, sock) {
    ...
    // Previously the return value was ignored, so reconnect errors were silently lost.
    const connWrapped = (update) => {
      Promise.resolve()
        .then(() => this._handleConnectionUpdate(sessionId, update, sock))
        .catch((e) => {
          console.error(
  ...
  async startAll() {
    ...
    for (const sid of keys) {
      ...
      // startDelayMs per-session delay (semaphore concurrency)
      if (this.startDelayMs > 0) {
        await new Promise((r) => setTimeout(r, this.startDelayMs));
      }
    }
