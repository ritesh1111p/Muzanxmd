...
// Prefer Baileys' public groupStatusMessage content type. Keep the older
// groupStatusMessageV2 relay as a compatibility fallback for builds that do
// not expose the content helper.
async function sendGroupStatus(message, storyData) {
  ...
  try {
    await sock.sendMessage(jid, { groupStatusMessage: storyData });
    return;
  } catch (err) {
    lastError = err;
  }
  ...
  throw lastError || new Error("WhatsApp rejected the group status message");
}
...
Module({
  ...
  package: "group",
  aliases: ["gs"],
  description: "Post a text, image, or video to the group status",
  usage: ".gstatus <text> | reply to image/video + .gstatus <caption>",
})(async (message, match) => {
  try {
    if (!(await checkPermissions(message))) return;

    const caption = String(match || "").trim();
    const quoted = message.quoted || null;
    const qType = quoted?.type || quoted?.mtype || "";

    if (quoted && ["imageMessage", "videoMessage"].includes(qType)) {
      await message.react("⏳").catch(() => {});

      const buffer = await quoted.download();
      if (!buffer?.length) {
        await message.react("❌").catch(() => {});
        return message.send("❌ _Could not download the replied media._");
      }

      const storyData =
        qType === "imageMessage"
          ? { image: buffer, caption: caption || quoted.caption || "" }
          : { video: buffer, caption: caption || quoted.caption || "" };
    ...
    if (!caption) {
      return message.send(
        "╭━━〔 ✦ 𝐆𝐒𝐓𝐀𝐓𝐔𝐒 ✦ 〕━━╮\n" +
        "┃ 📝 Text : .gstatus Hello everyone!\n" +
        "┃ 🖼️ Media: Reply to an image/video\n" +
        "┃ ➜ Then send: .gstatus <caption>\n" +
        "╰━━━━━━━━━━━━━━━━━━╯"
      );
    }
